...
var webUi = require('@trinsic/web-ui');
...
const TrinsicContext = React.createContext(undefined);
const TrinsicProvider = ({ children, sessionUrl, redirectUrl, exchangeUrl }) => {
    const [state, dispatch] = React.useReducer(trinsicReducer, initialState);
    const setConfig = React.useCallback((sessionUrl, redirectUrl, exchangeUrl) => {
    ...
    const createSession = React.useCallback(async (withRedirect = false) => {
        if (!state.sessionUrl) {
    ...
    const launch = React.useCallback(async (launchMode) => {
        if (!state.sessionUrl) {
            throw new Error('Session URL not set. Call useTrinsic with sessionUrl first.');
        }
        ...
        try {
            ...
            switch (launchMode) {
                case 'popup':
                    verificationResult = await webUi.launchPopup(async () => {
                        return await createSession();
                    });
                    break;
                case 'iframe':
                    const launchUrl = await createSession();
                    verificationResult = await webUi.launchIframe(launchUrl);
                    break;
                case 'redirect':
                    const redirectLaunchUrl = await createSession(true);
                    await webUi.launchRedirect(redirectLaunchUrl);
...
const useTrinsicPopup = (options) => {
    const context = useTrinsicBase(options);
    const popupLaunch = React.useCallback(async () => {
...
const useTrinsicRedirect = (options) => {
    const context = useTrinsicBase(options);
    const redirectLaunch = React.useCallback(async () => {
...
const useTrinsicIframe = (options) => {
    const context = useTrinsicBase(options);
    const iframeLaunch = React.useCallback(async () => {
...
exports.TrinsicProvider = TrinsicProvider;
exports.TrinsicTrigger = TrinsicTrigger;
exports.useTrinsic = useTrinsic;
exports.useTrinsicIframe = useTrinsicIframe;
exports.useTrinsicPopup = useTrinsicPopup;
exports.useTrinsicRedirect = useTrinsicRedirect;
